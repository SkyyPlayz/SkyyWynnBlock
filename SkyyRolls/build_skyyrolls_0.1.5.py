"""SkyyRolls 0.1.5 - build script (javassist via jpype). Item rolls (reforge + stats) in ItemStack metadata, shown on the tooltip,
...
for c, m in ((ITM, "getWeapon"), (IWP, "getBasicDamageBreakdown"), (IWP, "getUltimateDamageBreakdown"), (DBD, "entries"),
             ...
             (AC, "setPermissionGroups"), (AC, "requirePermission"), (IC, "setItemStackForSlot"), (INV, "getActiveToolsSlot"), (JP, "getDataDirectory"),
             (IS, "getQuantity")):
    B.probe(pool, c, m)
...
# 0.1.5: ONE source for the built-in cost ladder, the default reforge.properties text (Java defaultsText() below) and the config
...
# ================= 0.1.5 admin config kit (research/Server-Setup-Spec.md 4.2, tools/CONFIG-CONTRACT.md) =================
...
m = B.manifest("SkyyRolls", VERSION, "SkyWynn item rolls: reforge + stats stored in ItemStack metadata, shown on the tooltip as base value (+roll); /reforge Reforge Anvil page (coins via SkyyCoins, optional), costs editable in game (SkyWynn Menu Server Setup); /rolls admin tools. Zero dependencies.", PKG + ".SkyyRollsPlugin")

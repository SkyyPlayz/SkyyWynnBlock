...
public static String widgetSrc(String id, {PKG}.WLayout l) {{
  ...
  return "Group #w_" + id + " {{ Anchor: " + anchorSrc(l) + "; Background: #0b1524(0.72); Label #w_" + id + "_t {{ Anchor: (Full: 0); Style: (FontSize: " + fs + ", RenderBold: true, TextColor: #eaf6ff, Alignment: Center); Text: \\"\\"; }} }}";
}}""", wid))
...
public void build({REF} ref, {UCB} b, {UEB} ev, {ST} st) {{
  ...
  for (int i = 0; i < ids.length; i++) {{
    ...
    for (int a = 0; a < acts.length; a++) {{
      ev.addEventBinding({BT}.Activating, "#e_" + ids[i] + "_" + acts[a], {EVD}.of("a", ids[i] + ":" + acts[a]));
    }}
  }}
}}""", page))
page.addMethod(CtNewMethod.make(f"""
public void handleDataEvent({REF} ref, {ST} st, String data) {{
  try {{
    ...
    for (int i = 0; i < ids.length && wid2 == null; i++) {{
      for (int a = 0; a < acts.length; a++) {{
        if (data.indexOf(ids[i] + ":" + acts[a] + "\\"") >= 0 || data.indexOf("#e_" + ids[i] + "_" + acts[a] + "\\"") >= 0) {{ wid2 = ids[i]; act = acts[a]; break; }}
      }}
    }}
    if (wid2 == null) return;
...
B.assemble(jar, B.manifest("SkyyHud", VERSION, "SkyyHud: customizable server-side HUD. 8 widgets, per-player layouts, /skyyhud editor + layout codes. Zero dependencies.", PKG + ".SkyyHudPlugin"),
           OUT, {"Common/UI/Custom/Hud/SkyyHud.ui": HUD_UI, "Common/UI/Custom/Pages/SkyyHudEditor.ui": EDITOR_UI,

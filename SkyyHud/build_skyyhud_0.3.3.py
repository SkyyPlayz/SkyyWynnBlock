...
public static java.util.Map get(java.util.UUID u) {{
  ...
  for (int i = 0; i < ids.length; i++) {{
    ...
    if (!corner) {PKG}.Widgets.placeAtCell(w, {PKG}.Widgets.cellOf(w));
    if (w.dx < 0) w.dx = 0;
    if (w.dy < 0) w.dy = 0;
  }}
...
public HudCmd({PKG}.SkyyHudPlugin p) {{
  ...
  this.plugin = p;
  this.actionArg = withOptionalArg("action", "export | import | reset | profile | preview (omit to open the editor)", {ARG}.STRING);

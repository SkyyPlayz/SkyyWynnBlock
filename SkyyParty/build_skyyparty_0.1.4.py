"""SkyyParty 0.1.4 - build script (javassist via jpype).
Run:   python build_skyyparty_0.1.4.py            -> SkyyParty/SkyyParty-0.1.4.jar
       (--deploy exists like every Skyy build script, but only deploy with Skyy's OK; tools/deploy_set.py is the normal path)
Derived by COPY + EDIT from build_skyyparty_0.1.3.py (the live pin; no patch script, same as 0.1.3 itself). Same package, class names,
commands, bridge keys, file name and file keys. Everything 0.1.3 did is unchanged unless listed here.
...
# lines differ from 0.1.3's text; the loader still writes it with the system line separator, like 0.1.3.
CONFIG_TEXT = "\n".join([
...
# Row key = file key (the spec 4.5 names), so the file keys never change. Both rows are live: every reader of MAX / INVITE_MS reads the
# volatile field at click / tick time. No RELOAD routine: on a hand edit (found by /partyadmin reload, the menu's Reload file or the kit's
# next write) the kit writes the two fields itself and clamps to the same bounds as PartyStore.loadConfig (intProp 2-10 / 15-600).
CATS = [("party", "Party")]
...
KIT = CFG.emit(pool, PKG, MOD="SkyyParty", TITLE="Party", VERSION=VERSION, NODE="skyyparty.admin", CATS=CATS, ROWS=ROWS,
               FILES=["Skyy_SkyyParty/config.properties"], NOTE="Parties live in memory only. Both limits apply at once.",
...
public static String status() {
  return "[SkyyParty] maxSize=" + cur("maxSize", String.valueOf(@PKG@.PartyStore.MAX)) + " (most players in one party, 2-10), inviteSeconds="
    + cur("inviteSeconds", String.valueOf(@PKG@.PartyStore.inviteSecs())) + " (how long an invite stays open, 15-600)."
    + " Change: /partyadmin set <maxSize|inviteSeconds> <value|default>, or in game with /modconfig party."
...
public static String reload(@PR@ pr) {
  ...
  return "[SkyyParty] " + k + " Now: maxSize=" + @PKG@.PartyStore.MAX + ", inviteSeconds=" + @PKG@.PartyStore.inviteSecs() + ".";
}""")
M(adm, r"""
...
public PartyAdminSetCmd() {
  super("set", "(admin) Change a party setting: /partyadmin set <maxSize|inviteSeconds> <value|default>");
  requirePermission("skyyparty.admin");
  setPermissionGroups(new String[0]);
  this.keyArg = withRequiredArg("setting", "maxSize or inviteSeconds", @ATY@.STRING);
...
protected void execute(@CTX@ ctx, @ST@ store, @REF@ ref, @PR@ pr, @WLD@ world) {
  try {
    ...
    if (k == null || v == null) { pr.sendMessage(@MSG@.raw("Usage: /partyadmin set <maxSize|inviteSeconds> <value|default>")); return; }
...
public void setup() {
  ...
  // player Settings switches (research/Settings-Spec.md 2.2, General tab); party.invites is NOT built (it refuses - waits for Skyy)
  ...
  getLogger().at(java.util.logging.Level.INFO).log("[SkyyParty] """ + VERSION + r""" ready - /party page, invite|accept|decline|leave|list|kick|promote|disband, /pc, /partyadmin (admin); maxSize=" + @PKG@.PartyStore.MAX + " inviteSeconds=" + @PKG@.PartyStore.inviteSecs() + "; bridge party:fn:members, party:leader, party:name, party:stats; settings party.members, party.chat");
...
m = B.manifest("SkyyParty", VERSION, "SkyWynn parties: /party page (members with health, stamina, mana and where they are; invite box; leader kick / promote / disband), invites that run out (60 s by default), party chat /pc, leader handoff. Party size and invite time are set in game (SkyWynn Menu Server Setup, /partyadmin); players can hide party member news and party chat in /settings. Publishes party members + stats for the SkyyHud party widget. Zero dependencies.", PKG + ".SkyyPartyPlugin")

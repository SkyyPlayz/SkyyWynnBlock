...
| **Placement** | **Tier II, slot 5**: it replaces Quick Dodge (`RDodge`). The 12-node template is unchanged. It is unlocked at Acrobatics 10, max level 10. Level 1 = an air jump at 55 % of your jump height; level 10 = 100 %. |
...
| Tree totals at max | speed +20 % (unchanged), jump +0.7, fall -15 %, **dodge +10 % (was 20)**, Stamina +8 | speed **+15 %**, rest unchanged |

**Why tier II:** tier VI (the natural "capstone" slot) needs Acrobatics 60 = 111,672,425 XP. That is unreachable under the Acrobatics caps (the Tree-Feller-style capstone slot would hide the node from everyone). Tier II lets every player feel the node early. The 10 levels (55 % → 100 %) are the long grind. The loss (dodge push) is the smallest one in the tree, and Skyy only locked the Stamina node here ("a Stamina node in the Acrobatics tree; the rest ... figure out later", HANDOFF 1). **Open decision for Skyy** (section 6): tier II (now) or tier III (later).
...
6. **Old save lines:** when a player file has `Acrobatics.RDodge=<n>` (and/or `.off`) and no `Acrobatics.RDouble`, TreeStore load reads it as `RDouble`. The next save writes `RDouble` only. Same slot = same token and Dust cost, so nothing is refunded or lost. `trees.properties` gets `Acrobatics.RDouble.*` defaults through the same "append missing node lines" path 0.2 uses for its new trees. The 0.2 loader is UNVERIFIED (0.2 was not built when this was written); the build agent checks it. It is a no-op if 0.2 never shipped.
...
16. Migration: a 0.2 player file with `Acrobatics.RDodge=3` loads as Double Jump 3 and saves as `RDouble`.

---

## 6. Open questions for Skyy
1. **Placement:** tier II slot 5 (Acrobatics 10, replaces Quick Dodge; recommended) or tier III slot 7 (Acrobatics 20, replaces Sprinter)?

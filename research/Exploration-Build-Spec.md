...
| Spawner resolve | `builtin.blockspawner.BlockSpawnerPlugin$BlockSpawnerSystem.onEntityAdded`: `BlockSpawnerTable.getEntries()` (an `IWeightedMap`) `.get(HashUtil.random(x, y, z, WorldConfig.getSeed() + -1699164769L))` (`(JJJJ)D`, offsets 231-265), `CommandBuffer.removeEntity(ref, RemoveReason.REMOVE)`, `CommandBuffer.run(...)` with `BlockSpawnerEntry.getBlockComponents()` | bcfull |
...
**`chests.enabled=false`:** the capture above keeps running (a chest generated while it is off would otherwise be lost for good, its drop list cleared), but `ChestOpenSys.handle` queues nothing, the `ExpTick` window scan is skipped and `ExpAward.chestOpened` returns at once (so an `OpenCheck` already queued before a reload does nothing).
...
- Only ever inside step 5 of a first open, and never while `profile:busy` is present. `luck.enabled=false` skips this roll (the open still records and pays XP; Scavenger is not affected, its switch is the tree node).
...
### 2.5 A5: map coverage
In `ExpTick` (1 s, world thread), for each player (the whole chunk block is skipped, no record and no XP, while `chunks.enabled=false`):
...
In `ExpTick`, with the same skip rules as 2.5 (creative, flying); the whole zone block is skipped, no record and no XP, while `zones.enabled=false`:
...
bridge.retryMs=5000
```
Where each switch is read (all read live from `ExpCfg`, so `/exploreadmin reload` applies them): `chests.enabled` in 2.3 (award paths only, capture keeps running), `luck.enabled` in 2.4, `chunks.enabled` in 2.5, `zones.enabled` in 2.6. `noFlyingXp=false` drops the `flying` refusal / skip and `creativeXp=true` drops the creative one, in 2.3 step 1, 2.5 and 2.6 alike (gliding and mounting keep their own `chunks.*` switches). `titles.chatPrefix=false` makes `TitleFormatter.format` return the previous formatter's `m` unchanged (the hook stays registered). `titles.chatPriority` is read once in `setup()` for `registerAsyncGlobal`, so it needs a restart.
...
- Prefab chests with fixed preset items and no drop list (31 container instances in 7 prefab files, out of the 7,842 `*.prefab.json` files in Assets: 7,828 under `Server/Prefabs`, 14 under `Server/World`; all 31 are under `Server/Prefabs`) and empty world crates or barrels do not count. Only real loot chests do.
...
### 3.1 Slot and row
- `EXTRA_ROWS` gets `("Exploration", "Exploration", "Tool_Map", "#e0a040")` appended, so **slot 13** (append-only; existing slots unchanged). `must("Tool_Map")` passes (it is in Assets).
- **The two existing Python asserts right under the `SLOT_*` lists must be rewritten too** (otherwise the build stops with `AssertionError` before any class is made): `assert len(SLOT_NAMES) == len(SLOT_LABELS) == len(SLOT_ICONS) == len(SLOT_COLORS) == 13` becomes `... == 14`, and `assert SLOT_NAMES[10:] == ["Alchemy", "Smithing", "Cooking"] and SLOT_NAMES[5 + len(CLASS_ROWS) - 1] == "Combat.Mage"` becomes `SLOT_NAMES[10:] == ["Alchemy", "Smithing", "Cooking", "Exploration"]` (the Combat.Mage part stays). That also covers `SLOT_NAMES[13] == "Exploration"`. Update the `# 0.4: slots 10-12` comment to `10-13`.
...
- `PerkCfg.KEYS` gets `"exploration"` appended (9 keys). **Every 8-entry perk array gets a 9th entry, in two places:**
  - the static field defaults `HP`, `STA`, `MANA`, `DD`, `ONLY` (the `_decl` loop): `0.0` / `0.1` / `0.0` / `0.0` / `""`;
  - the five LOCAL fallback arrays inside `PerkCfg.read()`: `dhp`, `dsta`, `dmana`, `ddd`, `donly` get `0.0` / `0.1` / `0.0` / `0.0` / `""`. The loop runs `i < KEYS.length` (now 9) and evaluates `dhp[i]`, `dsta[i]`, `dmana[i]` unconditionally (and `donly[i]` whenever the key is absent), so an 8-entry local throws `ArrayIndexOutOfBoundsException` at `i = 8` on every `read()` (setup and every `/skills reload`). The build and the lint both pass anyway, so the patch script must `rep` these five lines explicitly (single asserted anchors, like every other `rep`), and after patching assert that no 8-entry `{{ 0.0, ... }}` / `{{ "", "_Trunk", ... }}` literal is left in `PerkCfg.read()`.
  - So `STA[8] = 0.1` (matching `perk.exploration.staminaPerLevel=0.1` below), the others 0. The existing 1 s `Perks.tick` sums it into the `skyyskill_stamina` MAX modifier. Coins per level: the generic `coinsPerLevel x level` (no change).
...
- `TREES += ["Acrobatics", "Exploration"]`, `TCOLOR += ["#c8a0ff", "#e0a040"]`, `NT = 6`, `N = 72`. Replace every literal 48: `TreeData.lv/off` arrays, `TreeCfg.EN`/`LIST` arrays, the asserts, and the `"of 48 nodes"` text (use `TreeDefs.N - TreeDefs.SOON_N`, see the SOON tally below). Assert `len(ROWS) == 72` and unique ids.
...
- **`SOON` kind (placeholder card):** never unlockable, never counted in "nodes on", state "Coming later" (card `#1a1a24` / `#8890a0`). Detail panel: `"Draft - Skyy designs the rest of the Exploration tree later"`. Buy button disabled: `"Coming later"`. No node lines in trees.properties. Hooks (0.1 names; `@K_SOON@` is the new kind token, `TreeDefs.soon(i)` = `KIND[i] == @K_SOON@`):
  - **Behaviour gate = `TreeCfg.EN`, baked false.** The `EN` field initializer is generated with `false` at every SOON index (not `["true"] * N`), and `TreeCfg.apply()` sets `en[i] = !TreeDefs.soon(i) && bool(p, k + "enabled", true)`, so no trees.properties line can turn one on. Through `EN`, `TreeCalc.eff()` returns 0 and `TreePage.canAct()` returns false with no further change.
  - `TreeCalc.state()`: first line `if (TreeDefs.soon(i)) return 0;` (before the owned / maxed checks, so a hand-edited `Exploration.ESoon5=` level never shows as owned). `TreeCalc.pathOk()` skips SOON indices in its loop.
  - Texts: `TreeCalc.lockShort()` returns `"Coming later"` right after the `d.bad` check (before `"Turned off"`), which also covers `TreePage.cardSub()`. `TreePage.buyText()`, `stateLine()`, `needLine()` and `nowLine()` return `"Coming later"` (the detail text above for `needLine`) as their first check.
  - Server side: `TreeOps.buy()` and `TreeOps.toggle()` refuse first with `"<name> is coming later"`.
  - Tally: `TreeCfg.load()` counts only non-SOON nodes and prints `"<on> of <N - soonCount> nodes on (<soonCount> coming later)"` (a generated `TreeDefs.SOON_N` constant, 8 in this draft).

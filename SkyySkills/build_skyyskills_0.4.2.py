"""SkyySkills 0.4.2 - build script (derived from 0.4.1 by tools/skills_0_4_2_patch.py - edit the patch, not this file;
...
Run:   python build_skyyskills_0.4.2.py            -> SkyySkills/SkyySkills-0.4.2.jar
       python build_skyyskills_0.4.2.py --deploy   -> also copies to Mods/SkyySkills.jar and enables it in the HUD mod world
...
defs.addField(CtField.make("public static final int CLASS_END = %d;" % (5 + len(CLASS_ROWS)), defs))
...
public static int argSlot({PR} pr, String arg) {{
  ...
  if (s < 0) {{
    pr.sendMessage({MSG}.raw("[Skills] Unknown skill. Use mining, foraging, farming, alchemy, smithing, cooking, acrobatics, exploration, combat (your class skill) or a class skill: archery, swordsmanship, assassination, shaman, sorcery."));
    return -1;
  }}
...
public TopCmd() {{
  ...
  this.skillArg = withRequiredArg("skill", "mining | foraging | farming | alchemy | smithing | cooking | acrobatics | exploration | combat (your class skill) | archery | swordsmanship | assassination | shaman | sorcery", {ATY}.STRING);
...
public XpCmd() {{
  ...
  this.skillArg = withRequiredArg("skill", "mining | foraging | farming | alchemy | smithing | cooking | acrobatics | exploration | combat (your class skill) | archery | swordsmanship | sorcery", {ATY}.STRING);
...
for c in (defs, cfg, sto, pub, fn, msg, fl, xp, plc, hg, btk, ptk, htk, bsy, psy, usy, ksy, tcm, top, page, tcmd, qcmd, rcmd, cmd, tick, pl, acfg, mvs_, acro, asy, afs,
          ...
          fcf, fdf, frd, fsn, fwt, fel, fdr, fcr, ffn, esy, pcg, pxp, pft):
    c.writeFile(OUT)
print("classes written")

jar = os.path.join(HERE, "SkyySkills-%s.jar" % VERSION)
m = B.manifest("SkyySkills", VERSION, "SkyWynn skills (Hypixel SkyBlock style): Mining, Foraging, Farming, Alchemy, Smithing, Cooking, Acrobatics, Exploration and your class combat skill (SkyyClasses) to level 100. XP from breaking blocks (a felled tree pays every log that falls), ripe crops, brewing at the Alchemy Bench, smelting at a Furnace, cooking (SkyyCooking), exploring (SkyyExploration - never boosted), kills with class weapons (nearby SkyyParty members get a share in their own class skill) and running / jumping / big survived falls / dodging (+ the Acrobatics tree Double Jump: crouch in mid-air). Alchemy makes potion effects last longer and adds max Mana; Exploration adds max Stamina. Perks: max health / stamina, double drops, class weapon damage; Acrobatics raises speed, jump height and dodge push and lowers fall damage (shared Skyy movement protocol). Stats page per skill. Skill tree bonuses and Tree buttons with SkyyTrees (optional). Level ups pay SkyyCoins. /skills. Per profile with SkyyProfiles (optional). Zero dependencies.", PKG + ".SkyySkillsPlugin")

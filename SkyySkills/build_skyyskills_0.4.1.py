"""SkyySkills 0.4.1 - build script (derived from 0.4 by tools/skills_0_4_1_patch.py - edit the patch, not this file;
...
Run:   python build_skyyskills_0.4.1.py            -> SkyySkills/SkyySkills-0.4.1.jar
       python build_skyyskills_0.4.1.py --deploy   -> also copies to Mods/SkyySkills.jar and enables it in the HUD mod world
...
public void setup() {{
  ...
  getLogger().at(java.util.logging.Level.INFO).log("[SkyySkills] {VERSION} ready - /skills; rows Alchemy (Alchemy Bench) / Smithing (Furnace) / Cooking (via SkyyCooking) / Exploration (via SkyyExploration, no boosters); bridge skill:fn:addxp + skill:fn:craftxp; trees bridge on (tree bonuses " + ({PKG}.BridgeCfg.BONUS ? "XP for " + {PKG}.BridgeCfg.BONUS_TEXT + " (grants from other mods: " + ({PKG}.BridgeCfg.BONUS_ADDXP_TEXT.length() > 0 ? {PKG}.BridgeCfg.BONUS_ADDXP_TEXT : "none") + ") + double drops" : "off") + ", SkyyTrees " + ({PKG}.SkillBonus.treesOn() ? "found" : "not loaded yet") + "); xp rules: " + rules + "; combat = class skill (SkyyClasses " + ({PKG}.SkillClass.allowedFn() != null ? "found" : "not loaded yet - no combat XP without it") + ")" + (fallOwner ? "" : "; fall damage bonuses are applied by " + {PKG}.MoveSync.bridge().get({PKG}.MoveSync.OWNER_FALL)));
}}""", pl))
pl.addMethod(CtNewMethod.make(f"""
...
m = B.manifest("SkyySkills", VERSION, "SkyWynn skills (Hypixel SkyBlock style): Mining, Foraging, Farming, Alchemy, Smithing, Cooking, Acrobatics, Exploration and your class combat skill (SkyyClasses) to level 100. XP from breaking blocks, ripe crops, brewing at the Alchemy Bench, smelting at a Furnace, cooking (SkyyCooking), exploring (SkyyExploration - never boosted), kills with class weapons and running / jumping / big survived falls / dodging. Alchemy makes potion effects last longer and adds max Mana; Exploration adds max Stamina. Perks: max health / stamina, double drops, class weapon damage; Acrobatics raises speed, jump height and dodge push and lowers fall damage (shared Skyy movement protocol). Stats page per skill. Skill tree bonuses and Tree buttons with SkyyTrees (optional). Level ups pay SkyyCoins. /skills. Per profile with SkyyProfiles (optional). Zero dependencies.", PKG + ".SkyySkillsPlugin")

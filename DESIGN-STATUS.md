...
- **Gear (`SkyyGear-Plan.md`, locked 2026-09-24, not built):** every gear item has a level requirement and a rarity tier. Higher Smithing = higher smithing rarity = better odds of a higher-rarity craft. Higher rarity = better reforge rolls. Mobs drop unidentified weapons and armor. Combat armor and weapons copy Wynncraft. Gathering gear is SkyBlock-style farming and foraging sets (mining likely the same). Wardrobe + loadouts save and quick-swap a set. Placeholders are fine; nothing on the inventory screen. **Stats are a mix:** Wynn's five skill points on weapons and armor, with crit chance, crit damage, and fortune layered on. The rest of the list, and how each applies in Hytale combat, are open. SkyyRolls 0.1.3 still shows rolls immediately and does not do unidentified drops, a rarity-based roll range, or this stat mix.
...
13. **Gear (open):** which level type gates a gear item (skill, class, or combat)? Exact rarity names and colours (Wynn's list vs SkyyRolls Common..Legendary)? Can drops reach the top rarities, or are some tiers craft-only? Where identification happens, how, and its cost? What a loadout saves (armor only vs weapons / accessories / HUD)? Powder slots and how powders drop? Bazaar vs auction house for rolled items? The stat mix is locked (five skill points, plus crit chance, crit damage, and fortune). Still open inside that lock: the exact stat list beyond those, and how each applies in Hytale combat.

"""SkyyIslands 0.3.1 - build script
...
for c, m in ((INS, "spawnInstance"), (INS, "teleportPlayerToLoadingInstance"), (INS, "teleportPlayerToInstance"), (INS, "exitInstance"),
             ...
             (CHU, "indexChunk"), (TC, "getTransform"), (TP, "createForPlayer"), (CTX, "provided"), (CEV, "setCancelled"), (EST, "getWorld"), (PR, "hasPermission"), (HSV, "SCHEDULED_EXECUTOR"), (PRE, "getPlayerRef"),
...
protected void execute({CTX} ctx, {ST} store, {REF} ref, {PR} pr, {WLD} world) {{
  ...
  try {{
    ...
    if (a.equals("visit") || a.equals("warp")) {{
      ...
      if (!{PKG}.IslandStore.isMember(target.getUuid(), u)) pr.sendMessage({MSG}.raw("[Island] Visiting " + target.getUsername() + "'s island - look, don't touch (they can /island invite you to build)."));
      return;
    }}
...
public void setup() {{
  ...
  getEntityStoreRegistry().registerSystem(new {PKG}.GuardSystem({DBE}.class));
  getEntityStoreRegistry().registerSystem(new {PKG}.GuardSystem({BBE}.class));
  getEntityStoreRegistry().registerSystem(new {PKG}.GuardSystem({PBE}.class));
  getEntityStoreRegistry().registerSystem(new {PKG}.GuardSystem({PUE}.class));
...
for c in (st_, fill, cfl, reln, relt, bld, icmd, hcmd, shub, rout, disp, rdy, seen, guard, pl):
    c.writeFile(OUT)

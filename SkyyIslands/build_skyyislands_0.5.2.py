...
# 0.5.2: the starter kit that 0.5.1 hard-coded in FillTask.starterKit (same items, order and amounts) = config starter.kit's default
...
# 0.5.2 player Settings registry (SkyyMenu 0.2+, research/Settings-Spec.md 1.3). No SkyyMenu = no answer = today's behaviour (on).
M(st_, r"""
...
public static void load(String animals) {
  ...
  } catch (Throwable t) { @PKG@.IslandStore.warn("could not read config.properties (the values in use stay - the defaults at startup): " + t); }
  if (!read) {
...
# 0.5.2: config.properties goes through the config kit's reload op (hand edits logged via=file; the RELOAD routine runs AFTER the kit's
...
# chat line to the island's owner and co-op members who are online (any profile), minus up to two players; 0.5.2: skey = a player Settings
...
public static int starterKit(@WLD@ world, @WCH@ chunk) {
  ...
  try {
    ...
    if (c == null) return -1;
    Object[] kit = @PKG@.IslandCfg.kit();   // 0.5.2: config starter.kit (default = the 0.5.1 kit, same order and amounts)
  ...
  try {
    ...
    if (owner != null && world.getName().equals(@PKG@.IslandStore.worldName(owner)) && !@PKG@.IslandStore.kitGiven(owner)) {
      ...
      if (kr >= 0) @PKG@.IslandStore.setKitGiven(owner);
        ...
        if (op != null && owner.equals(@PKG@.IslandStore.pkey(ou))) @PKG@.IslandStore.say(op, "[Island] Your starter kit is waiting: make room for " + @PKG@.IslandCfg.kitStacks() + " stacks in the chest your island started with (next to the spawn point), then use /island again.", "#ffe08a");
      }
    }
...
# 0.5.2: the /sethub logic, shared with the config kit's "Hub point" action (HubHereTask); same texts as 0.5.1, and a successful move is

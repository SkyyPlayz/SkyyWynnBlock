"""SkyyIslands 0.4.4 - build script
...
public static void info({PR} pr, {WLD} world) {{
  ...
  pr.sendMessage({MSG}.raw("[Island]" + {PKG}.IslandStore.profileLabel(u) + " " + (name == null ? "No island yet, /island to create one." : "world " + name + (({UNI}.get().getWorld(name) != null) ? " (loaded)" : " (unloaded)")) + "  members: " + (members.isEmpty() ? "none" : String.valueOf(members.split(",").length))));
...
public static void visit({ST} store, {REF} ref, {PR} pr, {WLD} world, {PR} target) {{
  ...
  if (!{PKG}.IslandStore.isMember(tk, pr.getUuid())) pr.sendMessage({MSG}.raw("[Island] Visiting " + target.getUsername() + "'s island - look, don't touch (they can /island invite you to build)."));
}}""", icmd))
...
protected void execute({CTX} ctx, {ST} store, {REF} ref, {PR} pr, {WLD} world) {{
  java.util.UUID u = pr.getUuid();
  try {{
    if (!ctx.provided(this.actionArg)) {{ go(store, ref, pr, world, {PKG}.IslandStore.pkey(u), true); return; }}
    String a = String.valueOf(ctx.get(this.actionArg)).trim().toLowerCase();
    if (a.equals("info")) {{ info(pr, world); return; }}
    if (a.equals("home") || a.equals("go")) {{ go(store, ref, pr, world, {PKG}.IslandStore.pkey(u), true); return; }}
...
m = B.manifest("SkyyIslands", VERSION, "SkyWynn private islands: /island creates and loads your own instanced sky island, /hub warps to the hub (/sethub), logins start in the hub, /island invite|visit for co-op, one island per profile (SkyyProfiles). Zero dependencies.", PKG + ".SkyyIslandsPlugin")

...
5. **SkyyGear build start (Skyy, 2026-09-25, answers to the builder's questions):** rarity ladder = **Wynn**: Normal (white), Unique (yellow), Rare (pink), Legendary (aqua), Fabled (red), Mythic (purple), plus Set (green). The gear **level requirement checks the player's class weapon skill** (Archer = Archery, Berserker = Fury, ...), for weapons AND armor; a too-high weapon is blocked with a popup, too-high armor gives no stats until the level is reached. **SkyyGear replaces SkyyRolls** (rolled items and /reforge move over; SkyyRolls retires). **Weapons/armor from mobs AND world chests drop unidentified**; gear players already own stays the lowest rarity with no modifiers until reforged (already-rolled items keep their rolls). Closes SkyyGear-Plan open questions 1 and 2.
6. **Gear level cap by gear type (Skyy, 2026-09-25):** the class level (class weapon skill) caps combat gear; the Mining level caps mining gear; the Foraging level caps foraging gear; the Farming level caps farming gear; and so on for later gathering skills. Every gear piece has a gate skill that its level requirement checks.
7. **Answers to OPEN-QUESTIONS (Skyy, 2026-09-25):** (a) the Settings switches for party invites, teleport requests and private messages **block** the sender when a player turns them off (not just hide). (b) **Base Mana 10** for everyone; **magic users (Mage, Priest) start at 20**. (c) **Overall Level** = the average of all your skills; every Overall Level gives a small amount of Health and Mana (this is the earlier 'Overall Level raises base Health and Mana' line, now to build). (d) **Priest weapons wait for custom weapons** (no wand/spellbook recipes now). (e) **Mining bag upgrades come from the Iron collection.** (f) SkyyEconomy merge after Skyy tests the separate mods.
8. **Bags use rarities, not sizes (Skyy, 2026-09-25):** Magic Bag tiers use the normal rarity ladder (Normal, Unique, Rare, Legendary) instead of Small / Medium / Large. A **Mythic Omni Bag** combines all the Legendary bags into one: it holds **100,000 of each item for every bag type**, the bag page keeps the bag types on separate tabs so it is easy to navigate, and with the Omni Bag you only need that one bag in your inventory.

"""Derive SkyySacks/build_skyysacks_0.6.4.py from 0.6.3.
...
dst = os.path.join(ROOT, "SkyySacks", "build_skyysacks_0.6.4.py")
...
rep('VERSION = "0.6.3"', 'VERSION = "0.6.4"')
s = s.replace('"""SkyySacks 0.6.3', '"""SkyySacks 0.6.4' + LF + '0.6.4: craft page pays by counted removal (fixes vanished furnace crafts), crafts.log, "< Back to bags".' + LF, 1)
BS = chr(92)
...
new = LF.join([
    ...
    '      if (extra > 0) {{',
    '        if (mq.getItemId() != null) {{ {SIC}.addOrDropItemStack(st, ref, p.getInventory().getCombinedBackpackStorageHotbar(), new {IS}(mq.getItemId(), extra)); audit.append("(returned ").append(extra).append(")"); }}',
    ...
    '        {SIC}.addOrDropItemStack(st, ref, p.getInventory().getCombinedBackpackStorageHotbar(), new {IS}(outs[k].getItemId(), (int) total));',
    '        given += (int) total;',
    '      }}',
    '    }}',

"""SkyyTrees 0.1 - build script (javassist via jpype, tools/skyybuild.py). NEW mod. Owner: Skyy (they/them).
...
Run:   python build_skyytrees_0.1.py            -> SkyyTrees/SkyyTrees-0.1.jar
...
for c, m in (("DBE", "getDamage"), ("DBE", "setDamage"), ("DBE", "getBlockType"), ("DBE", "isCancelled"),
             ...
             ("ACH", "getReferenceTo"), ("ST", "getComponent"), ("ST", "getExternalData"), ("EST", "getWorld"), ("WLD", "getName"),
...
TREES = ["Mining", "Foraging", "Farming", "Cooking"]
...
NODES = {
 "Mining": [
  ("MSpeed", "Mining Speed", "Tool_Pickaxe_Iron", "DMG", 25, 0.02, 0, "+%V breaking power on rock and ore", "Rock and ore blocks break in fewer hits (more damage per hit)", "", ""),
  ...
  ("MHeavy", "Heavy Pick", "Tool_Pickaxe_Mithril", "DMG", 20, 0.02, 0, "+%V breaking power on ore - adds to Mining Speed", "Ore blocks only - with both maxed ore takes x1.9 damage per hit", "", ""),
 ...
 "Foraging": [
  ("FSpeed", "Chopping Speed", "Tool_Hatchet_Iron", "DMG", 25, 0.02, 0, "+%V breaking power on wood", "Wood blocks break in fewer hits (more damage per hit)", "", ""),
  ...
  ("FSpeed2", "Chopping Speed II", "Tool_Hatchet_Mithril", "DMG", 20, 0.02, 0, "+%V breaking power on wood - adds to Chopping Speed", "Stacks with Chopping Speed", "", ""),
  ...
  ("FFeller", "Tree Feller", "Tool_Hatchet_Adamantite", "FELLER", 5, 8.0, 8.0, "Fells up to %V connected logs of a natural tree", "On a log that pays Foraging XP - the tree must touch leaves - cooldown %C s", "", ""),
 ],
 "Farming": [
...
assert len(ROWS) == 48 and len(set(r["id"] for r in ROWS)) == 48
...
DL = ["# SkyyTrees %s - skill trees for Mining, Foraging, Farming and Cooking. /tree reload re-reads this file (perm skyytrees.admin)." % VERSION,
      ...
      "ability.maxRadius=6",
      "vein.cooldownSec=40",
      "feller.cooldownSec=30", "feller.needLeaves=true", "feller.maxHeight=32",
...
for r in ROWS:
    tn = TREES[r["t"]]
    k = "%s.%s." % (tn, r["id"])
    DL.append("# %s S%d %s (tier %s)" % (tn, r["s"], r["name"], ["I", "II", "III", "IV", "V", "VI"][r["tier"] - 1]))
    DL.append(k + "max=%d" % r["max"])
    DL.append(k + "per=%s" % repr(float(r["per"])))
    if KINDS[r["kind"]] in ("VEIN", "FELLER"): DL.append(k + "base=%s" % repr(float(r["base"])))
    DL.append(k + "B=%d" % r["B"])
    DL.append(k + "tokens=%d" % r["tok"])
    DL.append(k + "enabled=true")
    if r["lk"]: DL.append(k + r["lk"] + "=" + r["lst"])
DEFAULTS = "\n".join(DL) + "\n"
...
F(defs, "public static final int NT = 4;")
...
public static String valueText(int i, double v) {
  int k = KIND[i];
  if (k == @K_HP@ || k == @K_STA@) return num(v);
...
for decl in ("int[] TIER_LV = new int[] { 1, 10, 20, 30, 45, 60 }", "int TOK_FIRST = 1", "int TOK_EVERY = 5", "long XP_PER_DUST = 10L",
             ...
             "int FELLER_H = 32", "long VEIN_CD_MS = 40000L", "long FELLER_CD_MS = 30000L", "boolean FELLER_LEAVES = true",
             ...
             "double[] BASE = @PKG@.TreeDefs.D_BASE", "int[] TOK = @PKG@.TreeDefs.D_TOK", "boolean[] EN = new boolean[] { " + ", ".join(["true"] * 48) + " }",
...
public static void apply(java.util.Properties p) {
  ...
  FELLER_CD_MS = clampL(lng(p, "feller.cooldownSec", 30L), 0L, 86400L) * 1000L;
...
public static synchronized String load() {
  ...
  apply(p);
  int on = 0;
  for (int i = 0; i < EN.length; i++) if (EN[i]) on++;
  int[] tl = TIER_LV;
  return on + " of 48 nodes on, tiers " + tl[0] + "/" + tl[1] + "/" + tl[2] + "/" + tl[3] + "/" + tl[4] + "/" + tl[5] + ", 1 Dust per " + XP_PER_DUST + " XP" + (EXTRA_TOKENS > 0L || EXTRA_DUST > 0L ? " (DEBUG extra tokens/Dust on)" : "");
}""")

# ================= TreeData: one profile's tree state =================
F(dat, "public int[] lv = new int[48];")
...
M(calc, r"""
public static long dustEarned(long xp) {
...
public static long tokSpent(@PKG@.TreeData d, int t) {
  long s = 0L;
  for (int k = 0; k < 12; k++) { int i = t * 12 + k; if (d.lv[i] >= 1) s = s + (long) @PKG@.TreeCfg.TOK[i]; }
  return s;
}""")
M(calc, r"""
...
public static double value(int i, int eff) {
  ...
  if (k == @K_VEIN@ || k == @K_FELLER@) return @PKG@.TreeCfg.BASE[i] + per * (double) eff;
...
public static void postTree(java.util.UUID u, @PKG@.TreeData d) {
  ...
  for (int t = 0; t < @PKG@.TreeDefs.NT; t++) {
    int owned = 0;
    for (int s = 0; s < 12; s++) if (d.lv[t * 12 + s] >= 1) owned++;
    if (t > 0) sb.append(',');
...
public static void stats(@CB@ cb, @REF@ ref, double[] v) {
  try {
    ...
    if (m == null) return;
    double hp = v[@I_FVIGOR@] + v[@I_AHEARTY@] + v[@I_CFED@];
    double sta = v[@I_MSTAMINA@];
...
public static boolean matches(String id, String match, boolean prefix) {
  ...
  return prefix ? id.startsWith(match) : id.equals(match);
}""")
...
public static boolean vein(@PR@ pr, String wn, int x, int y, int z, String id, int max) {
  ...
  @PKG@.TreeMsg.say(pr, "Vein Burst! +" + n + " ore (ready again in " + (cd / 1000L) + " s)", "#ffc300");
  return true;
}""")
M(abil, r"""
...
public static String needLine(@PKG@.TreeData d, int i, int lvl, long tokA, long dustA) {
  ...
  if (dustA < c) return "Needs: " + @PKG@.TreeDefs.grp(c - dustA) + " more Dust (1 per " + @PKG@.TreeCfg.XP_PER_DUST + " " + tn + " XP)";
  return "";
}""")
M(page, r"""
...
M(page, r"""
public static String note(@PKG@.TreeData d, int t, int lvl, long tokA, long dustA) {
  ...
  return "Tokens unlock nodes - Dust (1 per " + @PKG@.TreeCfg.XP_PER_DUST + " XP) levels them up - a respec gives everything back";
}""")
M(page, r"""
...
public void build(@REF@ ref, @UCB@ b, @UEB@ ev, @ST@ st) {
  ...
  for (int i = 0; i < @PKG@.TreeDefs.NT; i++) {
    b.appendInline("#SkyyTrHead", "TextButton #SkyyTrTab" + i + " { Anchor: (Width: 108, Height: 32); Text: \"" + @PKG@.TreeDefs.TREES[i] + "\"; " + (i == t ? "@BTON@" : "@BTOFF@") + " }");
    ev.addEventBinding(@BT@.Activating, "#SkyyTrTab" + i, @EVD@.of("a", "trtab" + i));
    b.appendInline("#SkyyTrHead", "Label { Anchor: (Width: 6, Height: 32); Text: \"\"; }");
  }
  b.appendInline("#SkyyTrHead", "Label #SkyyTrLvl { Anchor: (Width: 170, Height: 32); Text: \"\"; Style: (FontSize: 14, RenderBold: true, TextColor: " + col + ", HorizontalAlignment: End, VerticalAlignment: Center); }");
  b.appendInline("#SkyyTrHead", "Label #SkyyTrTok { Anchor: (Width: 150, Height: 32); Text: \"\"; Style: (FontSize: 13, RenderBold: true, TextColor: #ffe08a, HorizontalAlignment: End, VerticalAlignment: Center); }");
  b.appendInline("#SkyyTrHead", "Label #SkyyTrDust { Anchor: (Width: 190, Height: 32); Text: \"\"; Style: (FontSize: 13, RenderBold: true, TextColor: #c8a0ff, HorizontalAlignment: End, VerticalAlignment: Center); }");
  ...
  b.set("#SkyyTrNote.Text", note(d, t, lvl, tokA, dustA));
  ...
  line(b, "SkyyTrDetHow", @PKG@.TreeDefs.HOW[i0].replace("%C", cd(i0)), "#8fa6ba", 11, false, 52);
...
C(vcmd, r"""
public TreeSkillCmd() {
  super("Open one skill tree: /tree mining | foraging | farming | cooking");
...
protected void execute(@CTX@ ctx, @ST@ store, @REF@ ref, @PR@ pr, @WLD@ world) {
  ...
  if (want < 0) { pr.sendMessage(@MSG@.raw("[Trees] Unknown tree - use /tree mining, foraging, farming or cooking").color("#ff9090")); return; }
""" + open_body("want") + "\n}")
C(qcmd, r"""
public TreeQuietCmd() {
  super("quiet", "Toggle the 'Tree bonus' chat line (ability messages always show)");
  setPermissionGroups(new String[] { "hytale:Adventurer" });
}""")
M(qcmd, r"""
protected void execute(@CTX@ ctx, @ST@ store, @REF@ ref, @PR@ pr, @WLD@ world) {
  boolean q = @PKG@.TreeOps.quiet(pr);
...
C(cmd, r"""
public TreesCmd() {
  super("tree", "Open your skill trees: /tree, /tree mining | foraging | farming | cooking, /tree quiet");
...
public void setup() {
  ...
  getLogger().at(java.util.logging.Level.INFO).log("[SkyyTrees] @VERSION@ ready - /tree; " + sum + "; " + sk);
...
ALL = (defs, cfg, dat, sto, stk, calc, fx, msg, abil, gat, tfn, bfn, dmg, tick, svr, ops, page, vcmd, qcmd, rcmd, cmd, pl)
for c in ALL:
    c.writeFile(OUT)
print("classes written:", len(ALL))

jar = os.path.join(HERE, "SkyyTrees-%s.jar" % VERSION)
m = B.manifest("SkyyTrees", VERSION, "SkyWynn skill trees (Heart of the Mountain style) for Mining, Foraging, Farming and Cooking: tokens from your skill level unlock nodes, Dust from your skill XP levels them. Breaking power, double drops, XP, max health / stamina, gems, bars, saplings, seeds, extra drops, coins, tool speed, Spread, Vein Burst, Tree Feller and the Cooking bonuses SkyyCooking reads. Free respec. /tree. Per profile with SkyyProfiles (optional). Reads SkyySkills through the skyy bridge; zero dependencies.", PKG + ".SkyyTreesPlugin")

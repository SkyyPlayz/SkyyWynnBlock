...
Run:   python build_skyysacks_0.2.5.py            -> SkyySacks/SkyySacks-0.1.1.jar
       python build_skyysacks_0.2.5.py --deploy   -> also copies to Mods/SkyySacks.jar and enables it in the HUD mod world
...
public void build({REF} ref, {UCB} b, {UEB} ev, {ST} st) {{
  ...
  b.appendInline((String) null, "Group #SkyySacks {{ Anchor: (Width: 660, Height: 600); Background: #0b1524(0.96); Padding: (Horizontal: 16, Vertical: 10); LayoutMode: Top; }}");
  ...
  b.appendInline("#SkyySacks", "Label #SkyySCap {{ Anchor: (Height: 24); Text: \\"" + safe(this.cat + " sack   " + stored + " / " + capacity + (capacity == 0L ? "   (carry a " + this.cat + " sack to pool items)" : "")) + "\\"; Style: (FontSize: 14, RenderBold: true, TextColor: #ffe9c9, HorizontalAlignment: Center, VerticalAlignment: Center); }}");
...
protected void execute({CTX} ctx, {ST} store, {REF} ref, {PR} pr, {WLD} world) {{
  try {{
    ...
    player.getPageManager().openCustomPage(ref, store, new {PKG}.SacksPage(pr, (String) null));
  }} catch (Throwable t) {{
...
for cat in ("Mining", "Foraging", "Farming"):
    ...
    for tier, node in items.items():
        ...
        desc = "Holds up to %s %s items (%s). Items picked up into your storage are pooled automatically. Right-click to open. Carry several sacks to add their capacity." % (format(caps, ","), cat.lower(), what)

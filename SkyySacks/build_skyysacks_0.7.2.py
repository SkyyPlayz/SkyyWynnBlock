"""SkyySacks 0.7.2 - build script (javassist via jpype).
Run:   python build_skyysacks_0.7.2.py            -> SkyySacks/SkyySacks-0.7.2.jar
       python build_skyysacks_0.7.2.py --deploy   -> also copies to Mods/SkyySacks.jar and enables it in the HUD mod world
...
cpg.addField(CtField.make("public boolean liveOn;", cpg))
...
public void run() {{
  try {{
    ...
    for (int i = 0; wins != null && i < wins.size(); i++) {{
      ...
      if (consumed > 0) {PKG}.SackPool.save(k);
    }}
    if (!any) {{
      {PKG}.BagMirror m = ({PKG}.BagMirror) {PKG}.BagMirror.MIRRORS.get(k);
      if (m != null) {{ int c = m.sync(); if (c > 0) {PKG}.SackPool.save(k); {PKG}.BagMirror.MIRRORS.remove(k); }}
    }}
...
public java.util.ArrayList recipesFor(String tabId, java.util.UUID u) {{
  ...
  }} else if (tabId.equals("A:alch")) {{
    ids.addAll(benchRecipes(u, "Alchemybench"));
...
public static {CIC} materials({PLA} p, String k) {{
  {PKG}.BagMirror m = {PKG}.BagMirror.of(k);
  m.sync();
  m.rebuild({PKG}.SweepTask.caps(p.getInventory()));
...
public void live({REF} ref, {ST} st) {{
  ...
  java.util.UUID u = this.playerRef.getUuid();
  String k = {PKG}.SackPool.pkey(u);
  if (this.key == null || !this.key.equals(k)) return;
  String bench = this.tab.substring(2);
  {PKG}.ProcBench pb = {PKG}.ProcStore.get(k, bench);
  if (pb == null) return;
...
public void run() {{
  try {{
    ...
    if (pg instanceof {PKG}.CraftPage) {{
      ...
      if (cp.key != null && !cp.key.equals(k)) cp.profileNotice(k); else cp.live(r, st);
...
public void setup() {{
  ...
  {OCU}.registerSimple(this, {PKG}.SkyySacksPlugin.class, "SkyySacksAlchemy", new {PKG}.CraftPageFactory("A:alch"));
...
for c in (defs, sp, pjob, pben, pst, swp, tick, sav, cmp, page, cmd, fac, mir, clt, ctk, rcmp, clog, cpg, ccmd, cfac, ptk, ptick, pl):
    c.writeFile(OUT)
...
for cat in ("Mining", "Foraging", "Farming", "Combat"):
    ...
    for tier, node in items.items():
        ...
        what = {"Mining": "ore, rubble, rock and soil", "Foraging": "logs, planks, sticks, fibre and bark", "Farming": "plants, food, fish and life essence", "Combat": "bones, hides, feathers, essences, venom sacs, chitin and fabric scraps"}[cat]

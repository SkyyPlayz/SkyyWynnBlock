...
Run:   python build_skyysacks_0.5.0.py            -> SkyySacks/SkyySacks-0.1.1.jar
       python build_skyysacks_0.5.0.py --deploy   -> also copies to Mods/SkyySacks.jar and enables it in the HUD mod world
...
public void build({REF} ref, {UCB} b, {UEB} ev, {ST} st) {{
  ...
  if (this.cat == null) {{
    b.appendInline((String) null, "Group #SkyySacks {{ Anchor: (Width: 420, Height: 120); Background: #0b1524(0.96); Padding: (Horizontal: 16, Vertical: 12); LayoutMode: Top; }}");
    b.appendInline("#SkyySacks", "Label {{ Anchor: (Height: 30); Text: \\"Pocket Dimension\\"; Style: (FontSize: 16, RenderBold: true, TextColor: #ffe9c9, HorizontalAlignment: Center, VerticalAlignment: Center); }}");
    b.appendInline("#SkyySacks", "Label {{ Anchor: (Height: 44); Text: \\"You need a magic bag on you to reach in. Craft a Mining or Foraging or Farming bag at a workbench.\\"; Style: (FontSize: 11, TextColor: #c9b89a, HorizontalAlignment: Center, VerticalAlignment: Center, Wrap: true); }}");
  ...
  b.appendInline((String) null, "Group #SkyySacks {{ Anchor: (Width: 660, Height: 470); Background: #0b1524(0.96); Padding: (Horizontal: 16, Vertical: 10); LayoutMode: Top; }}");
  b.appendInline("#SkyySacks", "Group {{ Anchor: (Height: 2); Background: #e0b060; }}");
  b.appendInline("#SkyySacks", "Group #SkyySTabs {{ Anchor: (Height: 34); LayoutMode: Left; Padding: (Top: 6); }}");
  String[] cats = {PKG}.SackDefs.CATS;
  for (int c = 0; c < cats.length; c++) {{
    if (!caps.containsKey(cats[c])) continue;
    boolean sel = cats[c].equals(this.cat);
    b.appendInline("#SkyySTabs", "TextButton #SkyySTab" + cats[c] + " {{ Anchor: (Width: 130, Height: 28); Text: \\"" + cats[c] + "\\"; " + (sel ? tabOn : bs) + " }}");
    b.appendInline("#SkyySTabs", "Label {{ Anchor: (Width: 6, Height: 28); Text: \\"\\"; }}");
    ev.addEventBinding({BT}.Activating, "#SkyySTab" + cats[c], {EVD}.of("a", "tab:" + cats[c]));
  }}
  b.appendInline("#SkyySacks", "Label #SkyySCap {{ Anchor: (Height: 24); Text: \\"" + safe(this.cat + " bag - holds up to " + capacity + " of each item - " + stored + " stored") + "\\"; Style: (FontSize: 14, RenderBold: true, TextColor: #ffe9c9, HorizontalAlignment: Center, VerticalAlignment: Center); }}");
  ...
  this.cells = new String[36];
  for (int r = 0; r < 4; r++) {{
    b.appendInline("#SkyySacks", "Group #SkyySRow" + r + " {{ Anchor: (Height: 66); LayoutMode: Left; Padding: (Top: 4); }}");
    for (int c = 0; c < 9; c++) {{
      int idx = r * 9 + c;
      if (idx < entries.size()) {{
        ...
        this.cells[idx] = id;
        b.appendInline("#SkyySRow" + r, "Button #SkyySCell" + idx + " {{ Anchor: (Width: 62, Height: 62); Style: ButtonStyle( Default: ( Background: #1d3a5f ), Hovered: ( Background: #2f5a8f ), Disabled: ( Background: #1a2c3c ) ); ItemIcon {{ Anchor: (Width: 46, Height: 46, Left: 8, Top: 4); ItemId: \\"" + safe(id) + "\\"; }} Label {{ Anchor: (Width: 58, Height: 14, Right: 3, Bottom: 2); Text: \\"" + cnt + "\\"; Style: (FontSize: 11, RenderBold: true, TextColor: #ffffff, HorizontalAlignment: End); }} }}");
        ...
        this.cells[idx] = null;
        b.appendInline("#SkyySRow" + r, "Group {{ Anchor: (Width: 62, Height: 62); Background: #142030(0.9); }}");
      }}
      b.appendInline("#SkyySRow" + r, "Label {{ Anchor: (Width: 4, Height: 62); Text: \\"\\"; }}");
    }}
  }}
  b.appendInline("#SkyySacks", "Label #SkyySInfo {{ Anchor: (Height: 22); Text: \\"" + safe(this.info) + "\\"; Style: (FontSize: 12, TextColor: #c9b89a, HorizontalAlignment: Center, VerticalAlignment: Center); }}");
  b.appendInline("#SkyySacks", "Group #SkyySAct {{ Anchor: (Height: 36); LayoutMode: Left; Padding: (Top: 6); }}");
  b.appendInline("#SkyySAct", "TextButton #SkyySPickAll {{ Anchor: (Width: 150, Height: 28); Text: \\"Pick up all\\"; " + bs + " }}");
  b.appendInline("#SkyySAct", "Label {{ Anchor: (Width: 10, Height: 28); Text: \\"\\"; }}");
  b.appendInline("#SkyySAct", "TextButton #SkyySDepAll {{ Anchor: (Width: 150, Height: 28); Text: \\"Deposit all\\"; " + bs + " }}");
  b.appendInline("#SkyySAct", "Label {{ Anchor: (Width: 10, Height: 28); Text: \\"\\"; }}");
  b.appendInline("#SkyySAct", "Label {{ Anchor: (Width: 300, Height: 28); Text: \\"left click takes a stack - right click takes one\\"; Style: (FontSize: 11, TextColor: #8fa4b8, VerticalAlignment: Center); }}");
...
public void setup() {{
  ...
  getLogger().at(java.util.logging.Level.INFO).log("[SkyySacks] {VERSION} ready - /pd, right-click a magic bag; workbenches craft from your bags");
}}""", pl))
pl.addMethod(CtNewMethod.make(f"""
...
for c in (defs, sp, swp, tick, sav, cmp, page, cmd, fac, mir, clt, ctk, pl):
    c.writeFile(OUT)
...
for cat in ("Mining", "Foraging", "Farming"):
    ...
    for tier, node in items.items():
        ...
        what = {"Mining": "ore, rubble, rock and soil", "Foraging": "wood and logs", "Farming": "plants, food and fish"}[cat]

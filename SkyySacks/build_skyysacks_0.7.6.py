"""SkyySacks 0.7.6 - build script (javassist via jpype).
Run:   python build_skyysacks_0.7.6.py            -> SkyySacks/SkyySacks-0.7.6.jar
       python build_skyysacks_0.7.6.py --deploy   -> also copies to Mods/SkyySacks.jar and enables it in the HUD mod world
...
# 0.7.4: the Campfire tab text numbers = SkyyCooking's campfire.xpFactor / campfire.buffFactor defaults (cook:fn:campfire applies the real,
...
lang = []
# 0.7.5: the bag table (BAG_CATS / BAG_MAT / BAG_BOLTS / BAG_HOLDS near the top) - the first four rows are the 0.7.4 recipes unchanged
for cat in BAG_CATS:
    items = {
        "Small":  sack_item(cat, "Small", "Common",   [{"ItemId": BAG_BOLTS[0], "Quantity": BAG_BOLT_QTY}, {"ItemId": BAG_MAT[cat], "Quantity": BAG_MAT_QTY}]),
        "Medium": sack_item(cat, "Medium", "Uncommon", [{"ItemId": "Skyy_Sack_%s_Small" % cat, "Quantity": 1}, {"ItemId": BAG_BOLTS[1], "Quantity": BAG_BOLT_QTY}]),
        "Large":  sack_item(cat, "Large", "Rare",     [{"ItemId": "Skyy_Sack_%s_Medium" % cat, "Quantity": 1}, {"ItemId": BAG_BOLTS[2], "Quantity": BAG_BOLT_QTY}]),
    }
    for tier, node in items.items():
        ...
        caps = {"Small": CAPD["bag.small"], "Medium": CAPD["bag.medium"], "Large": CAPD["bag.large"]}[tier]
        what = BAG_HOLDS[cat]

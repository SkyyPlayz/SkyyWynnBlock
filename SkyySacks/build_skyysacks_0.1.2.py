"""SkyySacks 0.1.2 - build script (javassist via jpype).
Run:   python build_skyysacks_0.1.2.py            -> SkyySacks/SkyySacks-0.1.1.jar
       python build_skyysacks_0.1.2.py --deploy   -> also copies to Mods/SkyySacks.jar and enables it in the HUD mod world
...
ticker cancelled on shutdown; failures logged. Self-test join grant kept (strip once verified in-game).
"""
import sys, os
...
public void setup() {{
  ...
  getEventRegistry().registerGlobal({PRE}.class, new {PKG}.SackReady());
  ...
  getLogger().at(java.util.logging.Level.INFO).log("[SkyySacks] {VERSION} ready (self-test grant on join) - /sacks");
}}""", pl))
pl.addMethod(CtNewMethod.make(f"""
...
for c in (defs, sp, swp, tick, sav, cmp, page, cmd, grt, srd, pl):
    c.writeFile(OUT)

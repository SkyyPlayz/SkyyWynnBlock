...
Run:   python build_skyysacks_0.1.3.py            -> SkyySacks/SkyySacks-0.1.1.jar
       python build_skyysacks_0.1.3.py --deploy   -> also copies to Mods/SkyySacks.jar and enables it in the HUD mod world
...
public void setup() {{
  ...
  getLogger().at(java.util.logging.Level.INFO).log("[SkyySacks] {VERSION} ready (self-test grant on join) - /sacks or right-click a sack");
}}""", pl))
pl.addMethod(CtNewMethod.make(f"""
...
for c in (defs, sp, swp, tick, sav, cmp, page, cmd, grt, srd, fac, pl):
    c.writeFile(OUT)

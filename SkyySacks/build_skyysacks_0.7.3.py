"""SkyySacks 0.7.3 - build script (javassist via jpype).
Run:   python build_skyysacks_0.7.3.py            -> SkyySacks/SkyySacks-0.7.3.jar
       python build_skyysacks_0.7.3.py --deploy   -> also copies to Mods/SkyySacks.jar and enables it in the HUD mod world
...
public void build({REF} ref, {UCB} b, {UEB} ev, {ST} st) {{
  ...
  if (this.rows.size() == 0) b.appendInline("#SkyyCraft", "Label {{ Anchor: (Height: 30); Text: \\"" + {PKG}.SacksPage.safe(searching ? ("Nothing matches " + this.query + " in Crafting, Smithing or Farming.") : "No recipes here yet.") + "\\"; Style: (FontSize: 15, TextColor: #9fb8d0, HorizontalAlignment: Center, VerticalAlignment: Center); }}");
...
public void setup() {{
  ...
  getLogger().at(java.util.logging.Level.INFO).log("[SkyySacks] {VERSION} ready - /pd, /craft (Crafting, Smithing, Farming, search, timed Furnace/Tannery queues - Furnace pays Smithing XP, Collections), right-click a magic bag; benches craft from your bags (their ingredients first); storage per profile (pkey)");
}}""", pl))
pl.addMethod(CtNewMethod.make(f"""

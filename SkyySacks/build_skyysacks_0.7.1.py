...
public static void save(java.util.UUID u) {
  synchronized (SAVELOCK) { saveNow(u); }
}""", sp))
sp.addMethod(CtNewMethod.make("""
...
public void build({REF} ref, {UCB} b, {UEB} ev, {ST} st) {{
  ...
  if (this.tab != null && this.tab.startsWith("P:")) {{ buildProc(b, ev, p, u); return; }}
...
public void setup() {{
  ...
  getLogger().at(java.util.logging.Level.INFO).log("[SkyySacks] {VERSION} ready - /pd, /craft (Crafting, Alchemy, timed Furnace/Tannery queues), right-click a magic bag; workbenches craft from your bags");
}}""", pl))
pl.addMethod(CtNewMethod.make(f"""

"""SkyyEssentials 0.1.2 - build script (javassist via jpype). Derived from the LIVE 0.1.1 by copy + edit (this mod has no patch script).
Run:   python build_skyyessentials_0.1.2.py            -> SkyyEssentials/SkyyEssentials-0.1.2.jar
...
# ================= TCfg: config.properties (replyShortcut + the /trade keys), logger, bridge, atomic files, trade.log =================
# (key, label, type, java field, lo, hi, default, note shown after an in-game change, file comment)
ROWS = [
...
FILE_ORDER = [13] + list(range(0, 13))   # replyShortcut first (the 0.1.1 key), then the trade keys
N_ROWS = len(ROWS)
for r in ROWS:
...
F(tcfg, "public static final String[] NOTES = %s;" % jarr([r[7] for r in ROWS]))
...
# the generated get / put (one if per row; no String switch in javassist)
get_lines, put_lines = [], []
...
# typed in game: canonical text or null (REJECT, never clamp - research/Server-Setup-Spec.md 1.4.2)
M(tcfg, r"""
...
M(tcfg, r"""
public static String show(int i, String v) {
  String t = TYPES[i];
  if (t.equals("bool")) return "true".equals(v) ? "ON" : "OFF";
  return v;
}""")
M(tcfg, r"""
public static String rule(int i) {
  String t = TYPES[i];
  if (t.equals("bool")) return "use ON or OFF.";
  if (t.equals("mode")) return "use page or chest.";
  return "type a whole number from " + LO[i] + " to " + HI[i] + ".";
}""")
M(tcfg, r"""
public static String fileText() {
  StringBuilder sb = new StringBuilder();
  sb.append("# SkyyEssentials config - change it in game with /tradeadmin config (it writes this file at once),\n");
  sb.append("# or edit it here and run /tradeadmin reload. replyShortcut needs a server restart; everything else applies at once.\n");
  for (int k = 0; k < FILE_ORDER.length; k++) {
    int i = FILE_ORDER[k];
    if (k == 1) sb.append("#\n# ---- /trade (SkyyEssentials 0.1.2) ----\n");
    sb.append("# ").append(COMMENTS[i]).append("\n");
    sb.append(KEYS[i]).append("=").append(get(i)).append("\n");
  }
  return sb.toString();
}""")
M(tcfg, r"""
...
# null = written; else the reason
M(tcfg, r"""
public static synchronized String save() {
  try {
    atomicWrite(@ES@.CFG, fileText().getBytes("UTF-8"));
    return null;
  } catch (Throwable t) {
...
# missing file -> written with the defaults; unreadable -> BROKEN (never overwritten, in-game changes refused until it reads);
# keys missing from a readable file (a 0.1.1 file has only replyShortcut) -> the file is rewritten with every key, values kept
M(tcfg, r"""
public static synchronized String load() {
  try {
    ...
    for (int i = 0; i < KEYS.length; i++) {
      ...
      if (raw == null) { missing = true; continue; }
    ...
    if (missing) {
      String e = save();
      if (e == null) info("config.properties: added the missing keys (the /trade settings) - existing values kept");
    }
    return summary0();
  } catch (Throwable t) {
    BROKEN = true;
...
# in-game change (TCfgPage): validate (reject, never clamp), apply, write the WHOLE file at once (the file and the game always agree);
# a failed write puts the old value back. Returns a "+/-/=" line for the page.
M(tcfg, r"""
public static synchronized String set(int i, String raw, String who) {
  if (i < 0 || i >= KEYS.length) return "-Unknown setting.";
  if (BROKEN) return "-config.properties could not be read at the last load - fix or delete it, then click Reload file. Nothing was changed.";
  String c = canon(i, raw);
  if (c == null) return "-" + LABELS[i] + ": " + rule(i);
  String old = get(i);
  if (old.equals(c)) return "=" + LABELS[i] + " is already " + show(i, c) + ".";
  if (!put(i, c)) return "-Could not apply that value - nothing was changed.";
  String err = save();
  if (err != null) { put(i, old); return "-Could not write config.properties - nothing was changed (" + err + ")."; }
  log("CONFIG " + KEYS[i] + " " + old + " -> " + c + " by " + who);
  info("config " + KEYS[i] + " " + old + " -> " + c + " by " + who + " (in game)");
  String note = NOTES[i];
  return "+" + LABELS[i] + ": " + show(i, old) + " -> " + show(i, c) + " - saved to config.properties." + (note.length() > 0 ? " " + note : "");
}""")
...
# 7 /tradeadmin return (u1 = target, u2 = admin), 8 settings-page change (idx, val, by = 1 when typed, obj = the page, u2 = admin),
# 9 config reload (obj = the page or null for the command, u2 = admin), 10 write a record NOW (item moves, deliveries)
...
# 8 settings-page result (obj = the page, msg = result, side = row or -1, val = the typed text to keep on a refusal)
...
# read-only grid slots from a SNAPSHOT (never the live container); the real ItemStack gives the native item tooltip
M(tst, r"""
...
# ================= TCfgPage: /tradeadmin config - every key of config.properties, changed in game, written to the same file at once =================
F(tcpg, "public String info;")
F(tcpg, "public String[] keep;")
C(tcpg, r"""
public TCfgPage(@PR@ pr) {
  super(pr, @LIFE@.CanDismiss);
  this.info = ""; this.keep = new String[@PKG@.TCfg.KEYS.length];
}""")
# CustomUIPage.rebuild() is protected: TStore rebuilds this page (after the trade thread wrote config.properties) through this
M(tcpg, r"""
public void refresh() { rebuild(); }""")
M(tcpg, r"""
public String shown(int i) {
  String v = @PKG@.TCfg.get(i);
  String t = @PKG@.TCfg.TYPES[i];
  if (t.equals("bool")) {
    String s = "true".equals(v) ? "ON" : "OFF";
    if (@PKG@.TCfg.KEYS[i].equals("replyShortcut") && @PKG@.TCfg.REPLY_FILE != @ES@.R_SHORTCUT) s = s + " (restart)";
    return s;
  }
  return v;
}""")
...
public void build(@REF@ ref, @UCB@ b, @UEB@ ev, @ST@ store) {
  ...
  b.appendInline((String) null, "Group #SkyyTcfg { Anchor: (Width: 1060, Height: 900); Background: #0b1524(0.97); Padding: (Horizontal: 24, Vertical: 14); LayoutMode: Top; }");
  b.appendInline("#SkyyTcfg", "Group { Anchor: (Height: 3); Background: #6fd3a0; }");
  b.appendInline("#SkyyTcfg", "Label { Anchor: (Height: 46); Text: \"Trade settings\"; Style: (FontSize: 30, RenderBold: true, TextColor: #d8ffe8, HorizontalAlignment: Center, VerticalAlignment: Center); }");
  ...
  if (@PKG@.TCfg.BROKEN) {
    ...
    b.set("#SkyyTcBroken.Text", "config.properties could not be read - fix or delete it, then click Reload file. Changes are refused until then.");
  }
  ...
  int n = @PKG@.TCfg.KEYS.length;
  for (int i = 0; i < n; i++) {
    String row = "#SkyyTcRow" + i;
    String t = @PKG@.TCfg.TYPES[i];
    String v = @PKG@.TCfg.get(i);
    b.appendInline("#SkyyTcfg", "Group " + row + " { Anchor: (Height: 42); LayoutMode: Left; }");
    b.appendInline(row, "Label #SkyyTcLbl" + i + " { Anchor: (Width: 380, Height: 40); Text: \"\"; Style: (FontSize: 18, TextColor: #e6f2ff, VerticalAlignment: Center); }");
    b.set("#SkyyTcLbl" + i + ".Text", @PKG@.TCfg.LABELS[i]);
    b.appendInline(row, "Label #SkyyTcVal" + i + " { Anchor: (Width: 150, Height: 40); Text: \"\"; Style: (FontSize: 18, RenderBold: true, TextColor: #ffd37a, VerticalAlignment: Center); }");
    b.set("#SkyyTcVal" + i + ".Text", shown(i));
    if (t.equals("bool") || t.equals("mode")) {
      boolean on = t.equals("bool") ? "true".equals(v) : "page".equals(v);
      String a1 = t.equals("bool") ? "ON" : "page";
      String a0 = t.equals("bool") ? "OFF" : "chest";
      b.appendInline(row, "TextButton #SkyyTcOn" + i + " { Anchor: (Width: 110, Height: 36); Text: \"" + a1 + "\"; " + (on ? gs : ds) + " }");
      b.appendInline(row, "Label { Anchor: (Width: 10, Height: 36); Text: \"\"; }");
      b.appendInline(row, "TextButton #SkyyTcOff" + i + " { Anchor: (Width: 110, Height: 36); Text: \"" + a0 + "\"; " + (on ? ds : gs) + " }");
      b.appendInline(row, "Label { Anchor: (Width: 70, Height: 36); Text: \"\"; }");
      ev.addEventBinding(@BT@.Activating, "#SkyyTcOn" + i, @EVD@.of("a", "b1:" + i));
      ev.addEventBinding(@BT@.Activating, "#SkyyTcOff" + i, @EVD@.of("a", "b0:" + i));
    } else {
      b.appendInline(row, "Group #SkyyTcBox" + i + " { Anchor: (Width: 170, Height: 36); Background: #16263a; }");
      b.appendInline("#SkyyTcBox" + i, "TextField #SkyyTcV" + i + " { Anchor: (Full: 0); Padding: (Horizontal: 10); MaxLength: 18; PlaceholderText: \"new value\"; PlaceholderStyle: (TextColor: #6e7da1, FontSize: 16); Style: (TextColor: #ffffff, FontSize: 16); }");
      if (this.keep[i] != null && this.keep[i].length() > 0) b.set("#SkyyTcV" + i + ".Value", this.keep[i]);
      b.appendInline(row, "Label { Anchor: (Width: 10, Height: 36); Text: \"\"; }");
      b.appendInline(row, "TextButton #SkyyTcSet" + i + " { Anchor: (Width: 110, Height: 36); Text: \"Set\"; " + bs + " }");
      b.appendInline(row, "Label { Anchor: (Width: 10, Height: 36); Text: \"\"; }");
      ev.addEventBinding(@BT@.Activating, "#SkyyTcSet" + i, @EVD@.of("a", "set:" + i).append("@TcV" + i, "#SkyyTcV" + i + ".Value"));
      ev.addEventBinding(@BT@.Validating, "#SkyyTcV" + i, @EVD@.of("a", "set:" + i).append("@TcV" + i, "#SkyyTcV" + i + ".Value"), false);
    }
    b.appendInline(row, "Label #SkyyTcKey" + i + " { Anchor: (Width: 170, Height: 40); Text: \"\"; Style: (FontSize: 13, TextColor: #7f8ea6, VerticalAlignment: Center); }");
    b.set("#SkyyTcKey" + i + ".Text", @PKG@.TCfg.KEYS[i]);
  }
  ...
  b.set("#SkyyTcHelp0.Text", "Numbers: type a value, then press Enter or Set. Applies at once (offer slots: new trades only; /r: after a restart).");
  ...
  b.appendInline("#SkyyTcfg", "Label #SkyyTcInfo { Anchor: (Height: 32); Text: \"\"; Style: (FontSize: 17, RenderBold: true, TextColor: " + @PKG@.TradePage.colorOf(this.info) + ", HorizontalAlignment: Center, VerticalAlignment: Center); }");
  b.set("#SkyyTcInfo.Text", @PKG@.TradePage.textOf(this.info));
}""")
# (TCfgPage.handleDataEvent is compiled with the other bodies that call TStore, near the end)
...
# the settings page's change / reload was written on the trade thread: show the result on the page (one rebuild, in reaction to the
# admin's click) if it is still the open page, else in chat
M(tst, r"""
public static void cfgResultTask(@PKG@.TTask t, @PR@ pr, @REF@ ref, @ST@ st, @PLA@ p) {
  String res = t.msg == null ? "" : t.msg;
  if (!(t.obj instanceof @PKG@.TCfgPage)) { tellPr(pr, res); return; }
  @PKG@.TCfgPage pg = (@PKG@.TCfgPage) t.obj;
  pg.info = res;
  if (t.side >= 0 && t.side < pg.keep.length && t.val != null) pg.keep[t.side] = res.startsWith("-") ? t.val : null;
  Object cp = null;
  try { cp = p.getPageManager().getCustomPage(); } catch (Throwable e) { cp = null; }
  if (cp == pg) pg.refresh(); else tellPr(pr, res);
}""")
M(tst, r"""
...
# settings page / reload: config.properties is written on the trade thread (tmp + fsync + rename, retries), then the result goes back to
# the admin's world thread (TTask 8: page rebuild) - or into chat for /tradeadmin reload
M(tst, r"""
public static void cfgJob(@PKG@.TJob j) {
  String res = null;
  if (j.kind == 8) {
    res = @PKG@.TCfg.set(j.idx, j.val, j.byName);
  } else {
    String x = @PKG@.TCfg.load();
    @PKG@.TCfg.log("CONFIG reload by " + j.byName);
    res = (@PKG@.TCfg.BROKEN ? "-" : "+") + "Re-read config.properties: " + x;
  }
  if (j.obj == null) { tell(j.u2, res); return; }
  @PKG@.TTask t = new @PKG@.TTask(8, (@PKG@.TSession) null, j.kind == 8 ? j.idx : -1, j.u2, res);
  t.obj = j.obj;
  t.val = (j.kind == 8 && j.by == 1) ? j.val : null;
  hop(t);
}""")
M(tst, r"""
public static void cfgAsync(@PKG@.TCfgPage page, java.util.UUID au, String who, int kind, int idx, String val, boolean typed) {
  @PKG@.TJob j = new @PKG@.TJob(kind, (@PKG@.TRecord) null, (@PKG@.TSession) null, (String) null, typed ? 1 : 0, who);
  j.u2 = au;
  j.obj = page;
  j.idx = idx;
  j.val = val;
  java.util.concurrent.ScheduledExecutorService ex = SAVER;
  if (STOPPING || ex == null) { cfgJob(j); return; }
  try { ex.execute(j); } catch (Throwable t) { cfgJob(j); }
}""")
M(tst, r"""
public static void adminConfig(@PR@ pr, @REF@ ref, @ST@ st) {
  @PLA@ p = (@PLA@) st.getComponent(ref, @PLA@.getComponentType());
  if (p == null) { tellPr(pr, "-You are not in a world right now."); return; }
  p.getPageManager().openCustomPage(ref, st, new @PKG@.TCfgPage(pr));
}""")
M(tst, r"""
...
public static void jobRun(@PKG@.TJob j) {
  try {
    ...
    else if (j.kind == 8 || j.kind == 9) cfgJob(j);
...
# /tradeadmin config clicks: permission re-checked, then the change (or reload) is handed to the trade thread, which writes
# config.properties and sends the result back for ONE rebuild of this page (cfgResultTask) - no file I/O on the admin's world thread
M(tcpg, r"""
public void handleDataEvent(@REF@ ref, @ST@ store, String data) {
  try {
    if (data == null) return;
    ...
    try { allowed = this.playerRef.hasPermission(@PKG@.TCfg.ADMIN); } catch (Throwable t) { allowed = false; }
    if (!allowed) { this.info = "-You do not have permission to change trade settings (" + @PKG@.TCfg.ADMIN + ")."; rebuild(); return; }
    String who = this.playerRef.getUsername();
    java.util.UUID au = this.playerRef.getUuid();
    if (a.equals("reload")) { @PKG@.TStore.cfgAsync(this, au, who, 9, -1, (String) null, false); return; }
    int i = -1;
    String val = null;
    boolean typed = false;
    try {
      if (a.startsWith("b1:") || a.startsWith("b0:")) {
        ...
        boolean on = a.startsWith("b1:");
        if (i >= 0 && i < @PKG@.TCfg.KEYS.length && @PKG@.TCfg.TYPES[i].equals("mode")) val = on ? "page" : "chest";
        else val = on ? "true" : "false";
      } else if (a.startsWith("set:")) {
        i = Integer.parseInt(a.substring(4));
        val = @PKG@.TradePage.jsonStr(data, "@TcV" + i);
        typed = true;
      }
    } catch (Throwable t) { i = -1; }
    if (i < 0 || i >= @PKG@.TCfg.KEYS.length) return;
    @PKG@.TStore.cfgAsync(this, au, who, 8, i, val, typed);
  } catch (Throwable t) { @PKG@.TCfg.warn("trade settings click failed: " + t); }
}""")
M(tchg, r"""
...
tcmd("TAReloadCmd", "reload", "Admin: re-read config.properties", "admin",
     TS + "cfgAsync((@PKG@.TCfgPage) null, pr.getUuid(), pr.getUsername(), 9, -1, (String) null, false);")
...
# ================= plugin (0.1.1 setup / start / shutdown + the trade parts) =================
...
public void setup() {
  ...
  getLogger().at(java.util.logging.Level.INFO).log("[SkyyEssentials] @VERSION@ ready - /tpa /tpahere /tpaccept /tpdeny /tpacancel /msg (tell, w, whisper) /reply /fly; " + rNote + "; " + tNote);
}""")
...
OLD_CMDS = [tpa, tph, tacn, tac, tdnn, tdn, tcan, msg, rep, rcm, flc]
for c in [rq, es, hop, mv, rd, fly, tick] + OLD_CMDS + TRADE_ALL + TCMDS + [pl]:
    c.writeFile(OUT)
print("classes written:", 19 + len(TRADE_ALL) + len(TCMDS))

# compiled permission check: every command class of the jar references its permission call and argument
perm_of = {"FlyCmd": (b"requirePermission", b"skyyessentials.fly")}
...
m = B.manifest("SkyyEssentials", VERSION, "SkyWynn essentials, only what vanilla lacks: /tpa /tpahere /tpaccept /tpdeny /tpacancel (cross-world, 60s expiry), /msg /reply /r private messages (/r alone still redoes for builders), /trade (safe player trading: items + coins, escrow, countdown), admin /fly /tradeadmin. Zero dependencies.", PKG + ".SkyyEssentialsPlugin")

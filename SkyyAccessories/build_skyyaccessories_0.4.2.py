"""SkyyAccessories 0.4.2 - build script (derived from 0.4.1 by tools/acc_0_4_2_patch.py - edit the patch, not this file)
...
public void setup() {{
  ...
  getLogger().at(java.util.logging.Level.INFO).log("[SkyyAccessories] {VERSION} ready - /accessories, right-click the Accessory Bag; %d bench accessories + Omni (%d retired kept as items that do nothing - Alchemy Bench, Cooking Bench, Campfire), %d talismans in 5 rarities (percent layer on), one bag per profile when SkyyProfiles runs" );
...
assert len(omni_in) == len(ACTIVE) == 10

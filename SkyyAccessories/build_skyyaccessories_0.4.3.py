"""SkyyAccessories 0.4.3 - build script (derived from 0.4.2 by tools/acc_0_4_3_patch.py - edit the patch, not this file)
...
CAMPFIRE_DESC = ("Quick inventory cooking: campfire dishes in /craft at %d%% Cooking XP and %d%% of your cooking bonus. Put it in your "
                 "Accessory Bag and /craft cooks the Campfire recipes straight from your inventory - an emergency cook; a Cooking Bench "
                 "gives the full bonus and XP.") % (CAMP_XP_PCT, CAMP_BUFF_PCT)
assert CAMPFIRE_DESC.startswith("Quick inventory cooking: campfire dishes in /craft at 50% Cooking XP and 75% of your cooking bonus"), CAMPFIRE_DESC
def _camp_defaults_check():
    ...
    if not m:
        print("note: %s has no CAMP_BUFF_DEF / CAMP_XP_DEF line - Campfire description numbers not cross-checked" % os.path.basename(found[-1]))
        return
...
public void setup() {{
  ...
  getLogger().at(java.util.logging.Level.INFO).log("[SkyyAccessories] {VERSION} ready - /accessories, right-click the Accessory Bag; %d bench accessories + Omni (%d retired kept as items that do nothing - Alchemy Bench, Cooking Bench), Campfire accessory back for quick inventory cooking, %d talismans in 5 rarities (percent layer on), one bag per profile when SkyyProfiles runs" );

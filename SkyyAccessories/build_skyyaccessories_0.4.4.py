"""SkyyAccessories 0.4.4 - build script (derived from 0.4.3 by tools/acc_0_4_4_patch.py - edit the patch, not this file)
...
# ================= AccCfg (0.4.4): config.properties - loader, first-run file, the kit's RELOAD routine and check hook =================
...
public void build({REF} ref, {UCB} b, {UEB} ev, {ST} st) {{
  ...
  b.appendInline("#SkyyAcc", "Label #SkyyAccInfo {{ Anchor: (Height: 22); Text: \\"" + safe(this.info != null && this.info.length() > 0 ? this.info : {PKG}.AccStore.campLine(s)) + "\\"; Style: (FontSize: 12, TextColor: #c9b89a, HorizontalAlignment: Center, VerticalAlignment: Center); }}");
}}""", page))
...
public void setup() {{
  ...
  getLogger().at(java.util.logging.Level.INFO).log("[SkyyAccessories] {VERSION} ready - /accessories, right-click the Accessory Bag; %d bench accessories + Omni (%d retired kept as items that do nothing - Alchemy Bench, Cooking Bench), Campfire accessory back for quick inventory cooking, %d talismans in 5 rarities (percent layer on), one bag per profile when SkyyProfiles runs; " + cs + "; admin settings: SkyWynn Menu -> Server Setup -> Accessories or /accessories reload");
...
for c in (dfs, st_, cfg_, fn, page, fac, rcmd, cmd, tick, eff, pl, mvs_):
    c.writeFile(OUT)

...
# 0.2.2: player Settings registry (research/Settings-Spec.md 1.3; SkyyMenu 0.2+). No SkyyMenu = no answer = today's behaviour (on).
M(util, r"""
...
public static void creditOne(@PR@ pr, java.util.UUID u, String key, String item, long q, boolean admin) {
  try {
    ...
    if (paid[2] != 0L && @PKG@.CollUtil.notifyOn(u, "rewards.late")) pr.sendMessage(@MSG@.raw("   (some rewards are owed - they are paid automatically once SkyyCoins / SkyySkills accept them)").color("#c8b070"));
...
# ================= 0.2.2: the admin config kit (research/Server-Setup-Spec.md 4.11, tools/CONFIG-CONTRACT.md) =================

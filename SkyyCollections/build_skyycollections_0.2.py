"""SkyyCollections 0.2 - build script (written fresh; the 0.1.5 per-profile code, the coll:recipes bridge contract and the page
...
Run:   python build_skyycollections_0.2.py            -> SkyyCollections/SkyyCollections-0.2.jar
...
public static String loadConfig() {
  ...
  ADD_SOURCES = lowerSet(p.getProperty("bridge.add.sources", "skills:double"));
...
public void card(@UCB@ b, @UEB@ ev, String parent, int n, int c, @PKG@.CollData d, @PKG@.RegData R) {
  ...
  b.set("#SkyyCCdProg" + n + ".Text", "Tier " + @PKG@.CollUtil.roman(ct) + "    " + (ct >= mx ? "MAXED" : @PKG@.CollUtil.fmt(sm) + " / " + @PKG@.CollUtil.fmt(next)));
...
public void buildDetail(@UCB@ b, @UEB@ ev, @PKG@.CollData d, @PKG@.RegData R) {
  ...
  b.set("#SkyyCDTier.Text", "Tier " + @PKG@.CollUtil.roman(ct) + " of " + @PKG@.CollUtil.roman(mx) + (bt > ct ? "   (recipes bought up to tier " + @PKG@.CollUtil.roman(bt) + ")" : ""));
...
protected void execute(@CTX@ ctx, @ST@ store, @REF@ ref, @PR@ pr, @WLD@ world) {
  try {
    ...
    pr.sendMessage(@MSG@.raw("[Collections] reloaded: " + a + "; " + v));
...
public void setup() {
  ...
  getLogger().at(java.util.logging.Level.INFO).log("[SkyyCollections] 0.2 ready - item collections (" + s + "), /collections; migration + recipe check run at start");
}""")

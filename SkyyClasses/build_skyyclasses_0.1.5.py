...
Run:   python build_skyyclasses_0.1.5.py            -> SkyyClasses/SkyyClasses-0.1.5.jar
...
public void setup() {
  ...
  getLogger().at(java.util.logging.Level.INFO).log("[SkyyClasses] __VER__ ready - /class, /classadmin; classes " + @PKG@.ClassDefs.listText() + "; " + cfgText + " (coins bridge " + (@PKG@.ClassStore.coinsReady() ? "found" : "not found yet") + "; profiles " + (@PKG@.ClassCfg.profilesOn() ? "SkyyProfiles found - class per profile" : "SkyyProfiles not loaded yet - class per player") + "; config also in game: SkyWynn Menu -> Server Setup -> Classes)");
...
for c in (cfg, hooks, defs, st_, rul, afn, gfn, srec, strk, lock, page, opn, rtk, rdy, quit_, ctick, cmd, aset, ares, ainf, arel, adm, pl):
    c.writeFile(OUT)
kit.write(OUT)

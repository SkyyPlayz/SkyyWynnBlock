"""SkyyCoins 0.1.1 - build script (javassist via jpype).
Run:   python build_skyycoins_0.1.1.py            -> SkyyCoins/SkyyCoins-0.1.1.jar
       python build_skyycoins_0.1.1.py --deploy   -> also copies to Mods/SkyyCoins.jar and enables it in the HUD mod world
...
for c in (cs, cfg, task, tick, bal, pay, give, dp, pl):
    c.writeFile(OUT)
print("classes written")

jar = os.path.join(HERE, "SkyyCoins-%s.jar" % VERSION)
m = B.manifest("SkyyCoins", VERSION, "SkyWynn economy core: coin ledger, /balance, /pay, configurable death penalty (/deathpenalty 5% or 5%-10%). Zero dependencies.", PKG + ".SkyyCoinsPlugin")

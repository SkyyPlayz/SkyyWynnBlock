"""SkyyProfiles 0.1.1 - build script (javassist via jpype). SkyBlock-style profiles for SkyWynn = the class selector.
Run:   python build_skyyprofiles_0.1.1.py          -> SkyyProfiles/SkyyProfiles-0.1.1.jar
       (never --deploy: tools/deploy_set.py is the only deploy path; HANDOFF section 3)
Derived by copy + edit from build_skyyprofiles_0.1.py (0.1 has no patch script). Everything below the 0.1.1 block is the 0.1 design,
unchanged unless a line says 0.1.1.
...
ALL = (cfg, ros, nam, sto, pub, kfn, inv, sw, clr, page, opn, rec, pj, rtk, svt, rdy, pcon, quit_, pcre, psw, plst, pcmd, ainf, acls, arel,
       acfg, aset, adm, pl)
for c in ALL:
    c.writeFile(OUT)
